"""Carga dos JSONL produzidos pelos spiders em um DuckDB com esquema estrela.
...
def load(data_dir: Path, db_path: Path) -> dict[str, int]:
    ...
    try:
        ...
        if not present:
            raise SystemExit(f"Nenhum JSONL encontrado em {data_dir}/. Rode os spiders primeiro.")
